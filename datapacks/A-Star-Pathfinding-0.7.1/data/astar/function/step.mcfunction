...
execute if score @s astar.g >= maxDistance astar.config at b14fb83c-b441-457a-890f-4aabcfb6cb26 run return run execute as @n[type=marker,tag=astar,tag=astar.path] run function astar:mark_finalizer
...
scoreboard players add .max astar.step 1
scoreboard players operation @s astar.step = .max astar.step
